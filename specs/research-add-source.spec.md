...
- **Postagent 子进程**:
  - 命令来自 route template,例如
    `postagent send "https://api.github.com/..." -H "Authorization: Bearer $POSTAGENT.GITHUB.TOKEN"`
  - 不再为 public URL 自动加 `--anonymous`; public/doc/news/sports 页面应走 browser hand。
  - 期望 stdout 是原始响应体; HTTP 4xx/5xx 和网络诊断通过 stderr 的 `⚠ ...` 行解析。
  - `status` ≥ 400 → reason = `api_error`
  - 2xx 但 body 为空 / `{}` / `[]` → reason = `empty_content`
...
- **Actionbook 浏览器子进程**(3 步序列,MVP 串行):
  ...
  3. `actionbook browser text --session <session_id> --tab <tab_id> --json`
...
```json
{
  ...
  "data": {
    ...
    // accepted 时:
    "raw_path": "raw/1-github-issue-github.com.json",
    "trust_score": 2.0,
    // rejected 时:
...
场景: API 路径 add 一个 HN 源到 accepted + 完整 response envelope
  ...
  并且 `data.raw_path` == "raw/1-github-issue-github.com.json"
  并且 `<research_root>/s1/raw/1-<kind>-<host>.json` 存在且非空
