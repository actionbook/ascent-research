...
可复跑、可缓存、可引用的 deterministic evidence artifact 和 deterministic scorecard
report。LLM 不直接判断 fake star；CLI 先通过 GitHub API 和派生指标生成 `audit.json`，
并可用同一份 envelope 渲染专用 HTML 审计报告。research session 只能在该证据之上补充
上下文，不得替代评分与核心图表。
...
- 所有输出必须包含风险分数、风险等级、可信度分数、置信度、证据状态、触发原因和用于复核的 endpoint 摘要。
- `--out <path>` 写文件时必须写完整 JSON envelope，且 stdout 仍输出同一个 envelope 摘要。
- `--html <path>` 必须写专用 GitHub trust scorecard report，报告主体必须优先展示
  `trust.score`、`risk.score`、`risk.band`、`risk.confidence`、核心指标图表、reasons 和 evidence/gaps。
- GitHub trust HTML 不得依赖 LLM、`loop`、`finish` 或通用 research report 模板生成。
...
未传 `--json` 时仍走现有 envelope；人类可读输出只打印风险摘要、`--out` 路径和
`--html` 路径。
...
`risk.score` 范围是 0..=100，分数越高表示越可疑。`trust.score` 范围是 0..=100，
分数越高表示越可信，第一版由 `100 - risk.score` 派生。`risk.band` 和 `trust.band`
只允许 `low`、`medium`、`high`，不得用 `unknown` 占据等级字段。当样本不足、API 不可用
或 GitHub stats 仍在生成时，`risk.evidence_status="partial"` 且 `confidence` 降低；
命令仍可成功。

### 5. 专用 HTML scorecard

`--html` 渲染的 HTML 是审计结果视图，不是通用研究报告。它必须包含:

- 首屏 scorecard: repository、depth、stars/forks/issues/sample、`trust.score`、
  `trust.band`、`risk.score`、`risk.confidence`、`risk.evidence_status`。
- 指标图表: 至少展示 `trust.score`、`risk.score`、`risk.confidence`、`max_24h_star_share`、
  `max_daily_star_share`、`empty_bio_share`、`subscriber_star_ratio`；缺失指标显示
  `n/a`，不得编造。
- 指标表: 每个指标显示 key、value、observed count 和解释。
- reasons 表: 直接列出机器可读 `risk.reasons`。
- evidence/gaps 表: 展示 `risk.evidence` 和 `github_api.unavailable`。
- 结论边界: 明确写出 "Not a fake/real verdict"；当存在 unavailable/pending stats 时，
  解释 partial evidence 表示 GitHub 原生统计还不完整，而不是项目“不可信”。
...
单独的 star burst 不能直接判成中高风险；只有同时出现低质量账号、低仓库活跃、
低 fork/subscriber/contributor ratio 或账号创建时间集中等协同异常时，才显著提高风险分。
...
- 不要用通用 research HTML 模板替代 `github-audit --html` 的审计 scorecard。
...
场景: --html 写出 GitHub trust scorecard
  测试:
    包: ascent-research
    过滤: github_audit_html_report_renders_trust_scorecard
  层级: integration
  命中: packages/research/tests/github_audit.rs
  假设 timeline fixture 产生可计算的 burst、stargazer 和 repo ratio signals
  当 运行 `ascent-research --json github-audit owner/repo --depth timeline --sample 100 --html <tmp>/audit.html`
  那么 exit code 为 0
  并且 `<tmp>/audit.html` 存在
  并且 HTML 包含 "GitHub Trust Audit"
  并且 HTML 首屏包含 trust score、risk score、risk band、confidence 和 evidence status
  并且 HTML 图表包含 `trust.score`、`max_24h_star_share`、`max_daily_star_share`、`empty_bio_share`、`subscriber_star_ratio`
  并且 HTML 明确包含 "Not a fake/real verdict"
  并且 HTML 不包含 "Authorization" 或 "GITHUB.TOKEN"

场景: 自然传播 burst 不应拉低可信度
  测试:
    包: ascent-research
    过滤: github_audit_organic_launch_burst_keeps_high_trust_score
  层级: integration
  命中: packages/research/tests/github_audit.rs
  假设 timeline fixture 存在 24h 内高度集中的 star 增长
  并且 sampled stargazer 账号年龄分散且 followers/public repos/bio 正常
  并且 repo 有合理 fork/subscriber/contributor/activity 信号
  当 运行 `ascent-research --json github-audit owner/repo --depth timeline --sample 100`
  那么 `data.risk.score` 小于等于 10
  并且 `data.risk.band` 等于 "low"
  并且 `data.trust.score` 大于等于 90
  并且 `data.trust.band` 等于 "high"
...
- 不做确定性“真假 star”判决，只输出 trust score、risk score、risk band、confidence 和 evidence。
