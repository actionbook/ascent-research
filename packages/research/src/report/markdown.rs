...
fn resolve_diagram(
    src: &str,
    diagrams_root: &Path,
) -> Result<DiagramResolve, RenderError> {
...
fn apply_section_numbers(html: &str) -> String {
    ...
    re.replace_all(html, |caps: &regex::Captures| {
        ...
        format!(
            "<h{lvl}><span class=\"section-num\">{num}</span><span>{title}</span></h{lvl}>"
        )
    })
    .into_owned()
}
...
mod tests {
    ...
    fn section_number_wrapped() {
        let md = "## Overview\ncontent\n\n## 01 · WHY\nintro\n";
        let r = render(md);
        assert!(r.body_html.contains("<span class=\"section-num\">01</span>"));
    ...
    fn non_diagram_img_untouched() {
        ...
        let r = render_body(md, tmp.path()).unwrap();
        assert!(r.body_html.contains("<img src=\"https://example.com/pic.png\""));
